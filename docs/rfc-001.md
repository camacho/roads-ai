# RFC-001: Local-Only Teaching Harness

Status: Draft

## Summary

Define a local-first architecture that combines an open-source local model with an open-source harness while enforcing a teaching-oriented response policy: the system may explain concepts, show worked examples, and ask guiding questions, but it must not provide direct solutions for the learner to copy.

## Problem

Engineers learning to program often want immediate feedback without outsourcing all thinking to a hosted assistant. Existing tools tend to optimize for answer completion, broad tool access, and convenience rather than deliberate learning constraints.
